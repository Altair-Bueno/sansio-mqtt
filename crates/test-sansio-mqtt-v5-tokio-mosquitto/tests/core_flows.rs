...
async fn connect_and_disconnect() {
    ...
    let mut conn = Connection::connect(connect_options(port, "conn-disc"))
        .await
        .expect("connect");
    ...
    let event = conn.poll().await.expect("poll after disconnect");
    assert!(
...
async fn publish_qos0() {
    ...
    let mut conn_sub = Connection::connect(connect_options(port, "qos0-sub"))
        .await
    ...
    conn_sub.subscribe(sub("test/qos0")).expect("subscribe");
    ...
    let mut conn_pub = Connection::connect(connect_options(port, "qos0-pub"))
        .await
    ...
    // no response.
    let _ = tokio::time::timeout(Duration::from_millis(200), conn_pub.poll()).await;
...
async fn publish_qos1() {
    ...
    let mut conn_sub = Connection::connect(connect_options(port, "qos1-sub"))
        .await
        .expect("connect subscriber");
    assert!(matches!(
        conn_sub.poll().await.expect("subscriber connected"),
    ...
    conn_sub.subscribe(sub("test/qos1")).expect("subscribe");
    let sub_task = tokio::spawn(async move { conn_sub.poll().await });
    tokio::time::sleep(Duration::from_millis(150)).await;

    // Publisher at QoS 1 — expects PublishAcknowledged from broker.
    let mut conn_pub = Connection::connect(connect_options(port, "qos1-pub"))
        .await
        .expect("connect publisher");
    assert!(matches!(
        conn_pub.poll().await.expect("publisher connected"),
...
async fn publish_qos2() {
    ...
    // Subscriber at QoS 0.
    let mut conn_sub = Connection::connect(connect_options(port, "qos2-sub"))
        .await
        .expect("connect subscriber");
    assert!(matches!(
        conn_sub.poll().await.expect("subscriber connected"),
    ...
    conn_sub.subscribe(sub("test/qos2")).expect("subscribe");
    let sub_task = tokio::spawn(async move { conn_sub.poll().await });
    tokio::time::sleep(Duration::from_millis(150)).await;

    // Publisher at QoS 2 — full PUBLISH→PUBREC→PUBREL→PUBCOMP flow.
    let mut conn_pub = Connection::connect(connect_options(port, "qos2-pub"))
        .await
        .expect("connect publisher");
    assert!(matches!(
        conn_pub.poll().await.expect("publisher connected"),
    ...
    conn_pub
        ...
        .expect("publish");
    let pub_event = conn_pub.poll().await.expect("pubcomp");
    assert!(
...
async fn keep_alive_maintains_connection() {
    ...
    let mut conn = Connection::connect(opts).await.expect("connect");
    assert!(matches!(
        conn.poll().await.expect("connected"),
        Event::Connected
    ));
    ...
    let result = tokio::time::timeout(Duration::from_secs(7), conn.poll()).await;
    match result {
