...
async fn will_not_sent_on_graceful_disconnect() {
    ...
    let (sub_client, mut el_sub) =
        connect(connect_options(port, "wng-sub")).await.expect("connect sub");
    assert!(matches!(el_sub.poll().await.expect("poll"), Event::Connected));
    sub_client.subscribe(sub("will/ng")).await.expect("subscribe");
...
async fn will_sent_on_abrupt_disconnect_qos0() {
    ...
    let (sub_client, mut el_sub) = connect(connect_options(port, "waq0-sub")).await.expect("connect");
    assert!(matches!(el_sub.poll().await.expect("poll"), Event::Connected));
    sub_client.subscribe(sub("will/aq0")).await.expect("subscribe");
    tokio::time::sleep(Duration::from_millis(150)).await;
...
async fn will_sent_on_abrupt_disconnect_qos1() {
    ...
    assert!(matches!(el_sub.poll().await.expect("poll"), Event::Connected));
    sub_client.subscribe(sub_qos1("will/aq1")).await.expect("subscribe");
    tokio::time::sleep(Duration::from_millis(150)).await;
    let sub_task = tokio::spawn(async move { el_sub.poll().await });
...
async fn will_with_retain_flag() {
    ...
    let (sub_client, mut el_sub) = connect(connect_options(port, "wr-sub")).await.expect("connect");
    assert!(matches!(el_sub.poll().await.expect("poll"), Event::Connected));
    sub_client.subscribe(sub("will/retained")).await.expect("subscribe");
    ...
    let (cleanup_client, mut el_c) =
        connect(connect_options(port, "wr-cleanup")).await.expect("connect");
...
async fn will_with_delay_interval() {
    ...
    let (sub_client, mut el_sub) = connect(connect_options(port, "wdi-sub")).await.expect("connect");
    assert!(matches!(el_sub.poll().await.expect("poll"), Event::Connected));
    sub_client.subscribe(sub("will/delayed")).await.expect("subscribe");
    tokio::time::sleep(Duration::from_millis(150)).await;
...
async fn will_with_empty_payload() {
    ...
    let (sub_client, mut el_sub) =
        connect(connect_options(port, "wep-sub")).await.expect("connect");
    assert!(matches!(el_sub.poll().await.expect("poll"), Event::Connected));
    sub_client.subscribe(sub("will/empty")).await.expect("subscribe");
    tokio::time::sleep(Duration::from_millis(150)).await;
    let sub_task = tokio::spawn(async move { el_sub.poll().await });
...
async fn will_with_user_properties() {
    ...
    let (sub_client, mut el_sub) =
        connect(connect_options(port, "wup-sub")).await.expect("connect");
    assert!(matches!(el_sub.poll().await.expect("poll"), Event::Connected));
    sub_client.subscribe(sub("will/props")).await.expect("subscribe");
    tokio::time::sleep(Duration::from_millis(150)).await;
    let sub_task = tokio::spawn(async move { el_sub.poll().await });
